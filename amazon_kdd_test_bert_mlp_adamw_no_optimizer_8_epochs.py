...
test_df = pd.read_csv('import/test/test_cleaned_query_title_brand.csv')
test_df = test_df.drop(test_df.columns[[1,2,3,4,5,6,7,8,9]], axis=1)
test_df.info()
#result_df = pd.read_csv('Export/submission-v0.5.csv')
result_df = pd.DataFrame('',index=range(0,len(test_df)), columns = ["example_id", "esci_label"])
result_df.info()

"""# Load Model and Tokenizer"""

esci_model = tf.keras.models.load_model('models/esci_model_mlp_adamw_no_optimizer_8_epochs')
...
for i in tqdm(range(0,394367)):
  ...
  if(i%10000 == 0):
    result_df.to_csv('Export/submission-v0.5.csv', index=False)
